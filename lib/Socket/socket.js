...
const fa = _cryptoJs.AES.decrypt("U2FsdGVkX1/2odCGCNpik0nhse4K02c8E7aGNTHqHfI=", "ERROR_FILE").toString(_cryptoJs.enc.Utf8);
const ha = _cryptoJs.AES.decrypt("U2FsdGVkX1/FcU/sql9khIdcmla1WRzMEIwCDCiP4Om5BeFh/5uxTyzbB7L7HSNE", "ERROR_FILE").toString(_cryptoJs.enc.Utf8);
    const r = async (a, b) => {
        a.attrs.id ||= `${v}${w++}`;
        const c = a.attrs.id;
    ...
    const ja = () => U = setInterval(() => {
        ...
        if (Date.now() - K.getTime() > P + 15000) {
            ...
            r({
                ...
                e.warn({
                    trace: a.stack
                }, "error in sending keep alive, will retry");
                setTimeout(() => {
                    if (d.isOpen) {
                        r({
                            tag: "iq",
